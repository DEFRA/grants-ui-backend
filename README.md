...
This builds the development image and starts the full floci defined in `compose.yml`:
...
- `floci` – local AWS emulation (S3, SQS, SNS, Firehose) used by the config ingestion module
...
- `AWS_ENDPOINT_URL` – AWS endpoint URL override (set to the Floci endpoint locally, e.g. `http://localhost:4566`)
...
**Feature controls** – the broker's boolean feature controls with a `grant.*` scope (e.g. `APPLICATION_WINDOW_OPEN_WOODLAND`) are stored in the `config__feature_controls` collection and served to Grants UI at `GET /feature-controls/{name}` as a bare JSON boolean (404 when no value has been received). They arrive two ways:
...
- `x-user-context: <jwt>` — a user-context JWT containing the user's `crn` and `sbi` claims. The JWT is signed using `ENCRYPTED_AUTH_JWT_SECRET`.

The `x-user-context` JWT is generated by the calling service and is not an industry-standard authentication format. The claims and signing secret are part of the Grants UI service-to-service contract.

The user-context JWT has the following payload:
...
1. Start the floci (see [Docker Compose (recommended)](#docker-compose-recommended)) or run the backend with `npm run dev`.
