...
describe('feature-control repository', () => {
  ...
  test('returns null when no value has been stored', async () => {
    await expect(findFeatureControl('EXAMPLE_FEATURE_WOODLAND')).resolves.toBeNull()
  })
  ...
  test('stores and reads back a value', async () => {
    ...
    await expect(findFeatureControl('EXAMPLE_FEATURE_WOODLAND')).resolves.toEqual({
      value: false,
      valueType: 'boolean'
    })
  })
  ...
  test('upserting the same name replaces the value without creating a second document', async () => {
    ...
    await expect(findFeatureControl('EXAMPLE_FEATURE_WOODLAND')).resolves.toMatchObject({ value: true })
  })
})
