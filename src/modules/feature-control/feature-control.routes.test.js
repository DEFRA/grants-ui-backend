...
describe('featureControlValue route', () => {
  ...
  test.each([true, false])('returns 200 with the bare boolean %s', async (value) => {
    ...
    await featureControlValue.handler({ params: { name: 'APPLICATION_WINDOW_OPEN_WOODLAND' } }, mockH)
  ...
  test('looks the name up in upper case', async () => {
    ...
    expect(findFeatureControl).toHaveBeenCalledWith('APPLICATION_WINDOW_OPEN_WOODLAND')
  })
